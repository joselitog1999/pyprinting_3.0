# SYS-304: Arquitectura del Analizador SIF, Filtros 2D en Cascada y Ergonomía 🎛️

**PyPrinting 3.0 / PySpectrum 3.0 — Suite de Nanofotónica y Control Instrumental**  
**Laboratorio de Nanofotónica — Instituto de Nanosistemas (INS-UNSAM / CONICET)**  
**Autor Principal:** José Luis González Peñafiel (*Becario Doctoral CONICET*)  
**Código del Documento:** `SYS-304` | **Eje Temático:** `[DAT] (Procesamiento de Datos Espectrales y Ergonomía UI)`  
...
- **Módulos de Código Fuente:** `sif_analyzer.py`, `core/sif_processor.py`
...
| `test_06b` | **Transmisión Cascada 3 Fases** | Ejecución simultánea Despike + Wiener + SavGol en $T_{\text{calc}}$ | ✅ APROBADO |
| `test_06c` | **Panel Contextual de Filtros** | Adaptación dinámica de spinboxes, rangos y visibilidad por algoritmo | ✅ APROBADO |
| `test_06d` | **Atribución de Banda $\pm\sigma_T$** | Verificación analítica de que $\sigma_T$ pertenece a $T_{\text{calc}}$ bajo ISO/GUM | ✅ APROBADO |
...
**Resumen de la suite SIF Analyzer:** **17 de 17 pruebas superadas con éxito (100%)** en $10.8\ \text{s}$.
...
## 8. Arquitectura de Filtrado en Cascada (Enfoque A) y Panel Contextual en Transmitancia

Para resolver las limitaciones reportadas por los operadores sobre la inflexibilidad en el control de parámetros de suavizado, la redundancia en la selección de filtros y la imposibilidad de encadenar técnicas complementarias, se implementó el **Enfoque A** con un **Panel de Parámetros Sensible al Contexto** en la Ventana 4 (Transmisión).

### 8.1 Cadena de Procesamiento en Cascada de 3 Fases Físicas
La señal de transmitancia calculada $T_{\text{calc}}(\lambda)$ atraviesa un pipeline ordenado de forma secuencial y coherente:

```
[ T_calc Cruda (Ruta A o B) ]
             │
             ▼
[ FASE 1: Despiking en T(λ) ]  -->  Supresión de rayos cósmicos / spikes espurios (k_sigma = 4.0)
             │
             ▼
[ FASE 2: Wiener Adaptativo ]  -->  Denoising físico inverso basado en la PSD del Dark (alpha = 1.0)
             │
             ▼
[ FASE 3: Suavizado Espectral] -->  Filtro polinomial / frecuencial con parámetros contextuales
             │
             ▼
[ T_calc Limpia Final ]        -->  Se grafica y propaga a Extinción (Ventana 5)
```

1. **Fase 1 (Despiking Local):** Elimina transitorios de alta intensidad mediante la mediana local o el perfil adaptativo del sensor.
2. **Fase 2 (Wiener Físico Instrumental):** Aplica la deconstrucción analítica óptima en el dominio espectral usando la Densidad Espectral de Potencia experimental del ruido de fondo:
   $$H(\omega) = \frac{|S(\omega)|^2}{|S(\omega)|^2 + \alpha \cdot P_N(\omega)}$$
   *Se eliminó la redundancia previa donde Wiener figuraba como un checkbox independiente y a la vez como una opción dentro del combo de filtros.*
3. **Fase 3 (Suavizado Matemático Espectral):** Permite modelar la tendencia continua mediante métodos canónicos seleccionables.

### 8.2 Panel de Parámetros Sensible al Contexto
La interfaz gráfica implementa un controlador reactivo (`_on_trans_filter_type_changed`) que transmuta dinámicamente las etiquetas y campos de entrada según el algoritmo seleccionado:

| Algoritmo Seleccionado | Parámetro 1 Desplegado | Parámetro 2 Desplegado | Validaciones y Salvaguardas |
|---|---|---|---|
| **Savitzky-Golay** | `Ventana:` (QSpinBox impar, 3 a 101 px) | `Orden p:` (QSpinBox, 1 a 5) | Validador automático garantiza que el orden polinomial $p < \text{Ventana}$. Si el usuario introduce una ventana par, se redondea inmediatamente al siguiente entero impar. |
| **Fourier Lowpass** | `Corte fc:` (QDoubleSpinBox, 0.01 a 0.50) | *Oculto* | Frecuencia de corte acotada en fracción de Nyquist ($f_c \le 0.5$). |
| **Media Móvil** | `Ventana:` (QSpinBox, 2 a 51 px) | *Oculto* | Tamaño de núcleo deslizante simétrico. |
| **Ninguno** | *Oculto* | *Oculto* | Interfaz minimalista sin controles innecesarios. |

### 8.3 Clarificación Metrológica Fundamental: Origen de la Banda de Incertidumbre $\pm\sigma_T$ (ISO/GUM)
Un aspecto crucial esclarecido formalmente en esta revisión es la distinción física entre las dos curvas de transmitancia disponibles en la Ventana 4:

1. **Naturaleza Estática de $T_{\text{meas}}$:**  
   $T_{\text{meas}}$ corresponde al Canal 0 del archivo binario `.sif` generado internamente por Andor Solis durante la adquisición. Este vector representa los valores medios grabados por el software propietario sin ningún dato de dispersión, varianza o matriz de covarianza asociada. En consecuencia, **sobre $T_{\text{meas}}$ no es posible evaluar analíticamente una banda de incertidumbre física**.

2. **Atribución Exclusiva de $\pm\sigma_T$ a $T_{\text{calc}}$:**  
   La banda sombreada $\pm\sigma_T(\lambda)$ pertenece **única y estrictamente a $T_{\text{calc}}$**. Se evalúa mediante la ley analítica de propagación de varianzas estipulada en la norma internacional **ISO/IEC Guide 98-3 (GUM)**:
   ...
   donde:
   - $\sigma_L^2(\lambda) = \frac{L(\lambda)}{G} + \sigma_{\text{readout}}^2$ (ruido de disparo Poissoniano + ruido de lectura EMCCD).
   - $\sigma_R^2(\lambda) = \frac{R(\lambda)}{G} + \sigma_{\text{readout}}^2$.
   - $\sigma_D^2(\lambda) = \sigma_{\text{dark}}^2(\lambda)$ (varianza experimental del fondo térmico caracterizada en la Ventana 1).

En la interfaz gráfica, la banda se traza como un área semitransparente entre $[T_{\text{calc}}(\lambda) - \sigma_T(\lambda)]$ y $[T_{\text{calc}}(\lambda) + \sigma_T(\lambda)]$, otorgando rigor metrológico certificado a los resultados experimentales.

---

## 9. Guía Rápida de Operación para Investigadores y Becarios
