...
EQ_DIR = os.path.join(os.path.dirname(__file__), "eq_images")
...
def render_latex_eq(latex_str, filename, fontsize=13, color="#1A365D", dpi=300):
    ...
    # Calcular dimensiones para ReportLab
...
def build_pdf(filename="Incertidumbre_Metrologica_PyPrinting3.pdf"):
    ...
    eq1_flowable = get_eq_flowable(
        r"u_c(x_0) = \sqrt{u_{\mathrm{piezo}}^2 + u_{\mathrm{pix}}^2 + u_{\mathrm{fit}}^2 + u_{\mathrm{drift}}^2}",
        "eq1_uc.png", fontsize=14, color="#1A365D"
    )
    ...
    # Ecuación 2: PCov
    eq2_flowable = get_eq_flowable(
        r"u_{\mathrm{fit}}(x_0) = \sqrt{\mathrm{PCov}[x_0, x_0]} = \sqrt{\left(\mathbf{J}^T \mathbf{W} \mathbf{J}\right)^{-1}_{x_0, x_0}}",
        "eq2_pcov.png", fontsize=13, color="#1A365D"
    )
    ...
    # Ecuación 3: Fit SNR
    eq3_flowable = get_eq_flowable(
        r"u_{\mathrm{fit}}(x_0) \approx \frac{\mathrm{FWHM}}{\mathrm{SNR} \cdot \sqrt{N_{\mathrm{fotones}}}}",
        "eq3_fit_snr.png", fontsize=13, color="#1A365D"
    )
    story.append(KeepTogether([Spacer(1, 2), eq3_flowable, Spacer(1, 4)]))

    story.append(Paragraph(
        "<i>Para una nanopartícula brillante típica (FWHM = 260 nm, SNR = 40, N_fotones = 10,000), u_fit(x₀) es de tan solo <b>0.65 nm</b>.</i>",
        styles['CustomBody']
    ))
    ...
    # Ecuación 4: Pixelation
    eq4_flowable = get_eq_flowable(
        r"u_{\mathrm{pix}} = \frac{\Delta x}{\sqrt{12}} \approx 0.2887 \cdot \Delta x",
        "eq4_upix.png", fontsize=13, color="#1A365D"
    )
    ...
    # Ecuación 5: Varianza Intensidad
    eq5_flowable = get_eq_flowable(
        r"\sigma_Z^2 = \sigma_{\mathrm{shot}}^2 + \sigma_{\mathrm{dark}}^2 + \sigma_{\mathrm{laser}}^2 + \sigma_{\mathrm{ADC}}^2",
        "eq5_sigmaZ.png", fontsize=13, color="#1A365D"
    )
    ...
    # Ecuación 6: Shot Noise
    eq6_flowable = get_eq_flowable(
        r"\sigma_{\mathrm{shot}} = \sqrt{\bar{N}_{\mathrm{fotones}}} \propto \sqrt{V_{\mathrm{fotodiodo}}}",
        "eq6_shot.png", fontsize=12, color="#1A365D"
    )
    ...
    # Ecuación 7: ADC Noise
    eq7_flowable = get_eq_flowable(
        r"\sigma_{\mathrm{ADC}} = \frac{q}{\sqrt{12}} = \frac{0.305\ \mathrm{mV}}{\sqrt{12}} \approx 0.088\ \mathrm{mV} \quad (\mathrm{despreciable})",
        "eq7_adc.png", fontsize=12, color="#1A365D"
    )
    ...
    # Ecuación 8: Filter Threshold
    eq8_flowable = get_eq_flowable(
        r"Z_f[x, y] = Z_n[x, y] \quad (\mathrm{si}\ Z_n \geq \frac{P}{100}), \qquad Z_f[x,y] = 0.0 \quad (\mathrm{si}\ Z_n < \frac{P}{100})",
        "eq8_filter.png", fontsize=13, color="#1A365D"
    )
    ...
    # Ecuación 9: Disalignment
    eq9_flowable = get_eq_flowable(
        r"\Delta r_{\mathrm{nm}} = \sqrt{(x_1 - x_2)^2 + (y_1 - y_2)^2} \times 1000 \quad [\mathrm{nm}]",
        "eq9_disalignment.png", fontsize=13, color="#1A365D"
    )
    ...
    # Ecuación 10: Disalignment error propagation
    eq10_flowable = get_eq_flowable(
        r"u(\Delta r) = \sqrt{ \left(\frac{x_1 - x_2}{\Delta r}\right)^2 u(x_1)^2 + \left(\frac{y_1 - y_2}{\Delta r}\right)^2 u(y_2)^2 } \times 1000 \quad [\mathrm{nm}]",
        "eq10_udisalignment.png", fontsize=12, color="#1A365D"
    )
    ...
    print(f"DOCUMENTO PDF GENERADO CON ÉXITO Y ECUACIONES LATEX RENDERIZADAS: {filename}")
