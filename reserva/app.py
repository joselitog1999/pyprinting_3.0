...
class Frontend(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setGeometry(20, 20, 1440, 920)
        self._setup_menu()
        self._setup_docks()
    ...
    def _setup_docks(self):
        ...
        confocalDock = Dock("Confocal scan", size=(500, 400))
        ...
        self.dockArea.addDock(focusDock, "bottom", traceDock)

        nanoDock = Dock("Nanopositioning")
        self.nanoWidget = NanoFrontend()
        nanoDock.addWidget(self.nanoWidget)
        self.dockArea.addDock(nanoDock, "left", confocalDock)

        shuttersDock = Dock("Shutters / Flipper / Láser 532")
        self.shuttersWidget = ShuttersFrontend()
        shuttersDock.addWidget(self.shuttersWidget)
        self.dockArea.addDock(shuttersDock, "bottom", nanoDock)

        cameraDock = Dock("Cámara — Canon EOS", size=(640, 520))
        self.cameraWidget = CameraFrontend()
        cameraDock.addWidget(self.cameraWidget)
        self.dockArea.addDock(cameraDock, "right", confocalDock)
        ...
        # En modo seguro deshabilitar funciones que requieren hardware real
        if SAFE_MODE:
