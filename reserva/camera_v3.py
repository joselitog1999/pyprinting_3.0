...
class OverlayWidget(QWidget):
    ...
    zoomRectReadySignal = pyqtSignal()                # Shift+drag terminó → abrir zoom
    ...
    def _draw_rulers(self, p, W, H):
        ...
        if self._scale_set:
            h_val = f"{self._ruler_h_frac * H * self._um_per_px:.2f} µm"
            v_val = f"{self._ruler_v_frac * W * self._um_per_px:.2f} µm"
        else:
            h_val = f"{int(self._ruler_h_frac * H)} px"
            v_val = f"{int(self._ruler_v_frac * W)} px"
        p.drawText(vx + 5, 16, v_val)
        p.drawText(5, hy - 5, h_val)
    ...
    def _draw_measure(self, p, W, H):
        ...
        if len(pts) == 2:
            ...
            if self._scale_set:
                d_um = dist_px * self._um_per_px
                label = f"d={d_um:.3f}µm  θ={angle:.1f}°"
            else:
                label = f"d={dist_px:.1f}px  θ={angle:.1f}°"
            p.drawText(mx+6, my-6, label)
    ...
    def mousePressEvent(self, event):
        ...
        # Shift + click → iniciar ROI de zoom automáticamente
        ...
        # Modos especiales (activados por botones del toolbar)
        if self._mode == "roi":
            self._roi_start = (fx, fy)
            self._drag_roi  = True
            return
    ...
    def mouseReleaseEvent(self, _event):
        ...
        if self._drag_zoom:
            self._drag_zoom = False
            self._mode      = "none"
            # Señal para que CameraWindow abra ZoomWindow automáticamente
            self.zoomRectReadySignal.emit()
...
class SetScaleDialog(QDialog):
    ...
    def __init__(self, frame: np.ndarray, parent=None):
        ...
        # Distancia conocida — campo de texto libre en µm
        dist_row = QWidget()
        dist_lo  = QHBoxLayout(dist_row)
        dist_lo.setContentsMargins(0, 0, 0, 0)
        dist_lo.addWidget(QLabel("Distancia conocida (µm):"))
        self._dist_edit = QLineEdit("5.3")
        self._dist_edit.setFixedWidth(90)
        self._dist_edit.setPlaceholderText("ej: 5.3")
        self._dist_edit.setToolTip(
            "Ingresá la distancia real en µm entre los dos puntos seleccionados.")
        dist_lo.addWidget(self._dist_edit)
        dist_lo.addWidget(QLabel("µm"))
        ...
        self._dist_edit.textChanged.connect(self._update_result)
    ...
    def _update_result(self):
        ...
        if dist_px < 1:
            return
        try:
            known_um = float(self._dist_edit.text().replace(",", "."))
        except ValueError:
            self._result_lbl.setText("⚠ Valor inválido — ingresá un número (ej: 5.3)")
            self._result_lbl.setStyleSheet("font-family: monospace; color: #e5534b;")
            self._ok_btn.setEnabled(False)
            return
        if known_um <= 0:
            self._result_lbl.setText("⚠ La distancia debe ser mayor que 0")
            self._result_lbl.setStyleSheet("font-family: monospace; color: #e5534b;")
            self._ok_btn.setEnabled(False)
            return
...
class TrackpyDialog(QDialog):
    ...
    Parámetros expuestos: diameter, separation, threshold.
    ...
    def __init__(self, frame: np.ndarray, roi_frac: Optional[tuple] = None, parent=None):
        ...
        # Visor + overlay de partículas detectadas
        self._view = pg.GraphicsLayoutWidget()
        ...
        # Parámetros (solo los tres expuestos)
        params_box = QGroupBox("Parámetros trackpy")
        form = QFormLayout(params_box)

        self._diam = QSpinBox()
        self._diam.setRange(3, 201)
        self._diam.setSingleStep(2)
        self._diam.setValue(11)
        self._diam.setToolTip(
            "Tamaño estimado de la partícula en píxeles. Debe ser impar.\n"
            "Si ponés un número par se redondea al siguiente impar.")

        self._sep = QDoubleSpinBox()
        self._sep.setRange(1, 500)
        self._sep.setValue(8)
        self._sep.setDecimals(1)
        self._sep.setToolTip(
            "Separación mínima entre dos partículas en píxeles.\n"
            "Partículas más cerca que esto se fusionan en una sola.")

        self._thr = QDoubleSpinBox()
        self._thr.setRange(0, 1e6)
        self._thr.setValue(0)
        self._thr.setDecimals(1)
        self._thr.setSingleStep(10)
        self._thr.setToolTip(
            "Umbral mínimo de masa integrada para aceptar una partícula.\n"
            "0 = sin umbral (usa percentile interno de trackpy).\n"
            "Subilo si aparecen muchos falsos positivos.")
...
class CameraWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self._overlay.zoomRectReadySignal.connect(self._open_zoom_window)
        ...
        # Indicador de zoom (Shift+arrastrar) — no hay botón, es un atajo directo
        zoom_hint = QLabel("  Zoom: Shift+arrastrar")
        zoom_hint.setStyleSheet("color: #ffc832; font-size: 11px;")
        tb_lo.addWidget(zoom_hint)
        tb_lo.addStretch()
    ...
    def _update_guards(self):
        """
        Reglas, Medir y Zoom siempre habilitados (operan en píxeles por defecto).
        ROI → Confocal requiere: referencia fijada + escala calibrada.
        """
        ...
        if not scale_needed:
            ...
            if not self._scale_set:
                tip.append("Set scale")
            self._btn_confocal.setToolTip(
                f"Requiere: {' y '.join(tip)} antes de enviar a Confocal.")
            ...
            if len(self._measure_pts) == 2 and self._current_frame is not None:
                ...
                if self._scale_set:
                    ...
                    self._lbl_result.setText(
                        f"Distancia: {dist_um:.4f} µm  ({dist_px:.1f} px)   |   Ángulo: {angle:.2f}°")
                else:
                    self._lbl_result.setText(
                        f"Distancia: {dist_px:.1f} px   |   Ángulo: {angle:.2f}°  "
                        f"[sin calibrar — hacé Set scale para obtener µm]")
    ...
    def _run_detection(self):
        ...
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                df = tp.locate(gray, diameter=d,
                               separation=p.get("separation", 8),
    ...
    def _open_zoom_window(self):
        """
        Abre (o actualiza) la ventana de zoom con el ROI actualmente dibujado.
        Se llama automáticamente al soltar Shift+arrastrar (señal zoomRectReadySignal).
        """
        zr = self._overlay._zoom_rect
        if zr is None or self._current_frame is None:
            return
        if self._zoom_window is None:
            self._zoom_window = ZoomWindow(parent=self)
        self._zoom_window.update_frame(self._current_frame, zr, self._um_per_px)
