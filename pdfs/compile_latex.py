# -*- coding: utf-8 -*-
"""
compile_latex.py — Generador y compilador del documento LaTeX nativo (.tex)
para MiKTeX / TeX Live — PyPrinting 3.0
Ubicación: pdfs/compile_latex.py
UNSAM — Instituto de Nanosistemas (INS-UNSAM)
"""
...
TEX_FILENAME = os.path.join(BASE_DIR, "Incertidumbre_Metrologica_PyPrinting3.tex")
...
def main():
    print(f"Escribiendo código LaTeX nativo en {TEX_FILENAME}...")
    with open(TEX_FILENAME, "w", encoding="utf-8") as f:
        f.write(LATEX_CONTENT.strip())

    print("Compilando documento LaTeX nativo con MiKTeX (pdflatex) dentro de pdfs/...")
    cmd = ["pdflatex", "-output-directory=" + BASE_DIR, "-interaction=nonstopmode", TEX_FILENAME]
    res = subprocess.run(cmd, capture_output=True, text=True)

    if res.returncode == 0:
        # Segunda pasada para referencias cruzadas y número de páginas
        subprocess.run(cmd, capture_output=True, text=True)
        print(f"¡ÉXITO PERFECTO! PDF LaTeX generado en: {PDF_FILENAME}")
    else:
        print("Error en la compilación de pdflatex:")
